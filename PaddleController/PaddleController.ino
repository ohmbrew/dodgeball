/*
  ...
  LattePanda Arduino pinout
                      ...
                      @           @     @
    *     *     *     *
   E2A   E2B   E1A   E1B   SR1   SL1  PWM1  POT1    
   D0    D1    D2    D3    D4    D5    D6    A0    D19   D20    5V    5V
    |     |     |     |     |     |     |     |     |     |     |     | 

    |     |     |     |     |     |     |     |     |     |     |     |   
   D7    D8    D9    D10   D11   D12   D13   A3    D22   D23   GND   GND
  DIR1  DIR2        PWM2   SR2   SL2        POT2 
    *           @     @     @           @

  * = digital interrupt capable. Prefer to put motor encoder signals on these pins to get encoder values in real-time without missing edges
  @ = PWM capable pin. Motor PWM signals must be on these pins

  Player 1 signals:
  POT1  [A0]: Pot P1 - center tap of pot connected to this pin, other wires to gnd and 5V
  SL1   [D5]: Limit Switch Left P1
  ...
  Player 2 signals:
  POT2  [A3]: Pot P2 - center tap of pot connected to this pin, other wires to gnd and 5V
  SL2   [D12]: Limit Switch Left P2
  DIR2  [D8]: Motor Direction P2
  PWM2 [D10]: Motor PWM P2
  E2A   [D0]: Motor Encoder A P2
  E2B   [D7]: Motor Encoder B P2
  SR2  [D11]: Limit Switch Right P2


  Operation
  - potentiometer control for paddle left/right
    - Control paddle by using a pot that has left/right limits. Those limits equate to paddle far left/far right. As you move pot, it moves paddle to that position
    - center pin of the potentiometer to the analog input pin (A0/A3 for P1/P2 respectively). Left pin of pot if knob faces you -> GND. Right pin to +5V
  - drive 2x pins (pwmPinP1 and pwmPinP2) with AnalogWrite (see below) to drive PWM signal to H-Bridge chip to motor
  - motor encoder signals (2 signals per motor = 4) going to inputs (M1=[D2,D3] and M2=[D0,D7]) used to calculate position and send that data over serial port to MPF
  - regularly send paddle position info over usb-to-serial connection for MPF python custom module (base.py) to listen to and send events accordingly
  - regulary receive info like setting paddle position in the case of AI player 2

  
  Notes:
  - Using example Analog Serial Monitoring, pot currently reports 0 for full turned one way, 1023 for full turned other way. So use that as scale for PWM motor control
...
/*
  ...
  - move motor right until right limit switch hit
  - Set max position
  - when both paddles have done this, they'll move to whatever the pot is in
*/
